...
def pixelpackbufferboundread_pixels_mutator_match(program, api, log_message,
                                                  **kwargs):
    return "pixel_pack buffer bound" in (
        log_message.message.lower() if log_message.message is not None else "")


class PixelpackbufferboundreadPixelsMutator(BaseMutator):
    def __init__(self):
        self.name = "PixelpackbufferboundreadPixelsMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(6,
                        api.args[6].arg_type.gen_value(None, api, api.args[6]))
        print("PixelpackbufferboundreadPixels")
        return True

...
_api_names = "getActiveUniform"


def uniformsget_active_uniform_mutator_match(program, api, log_message,
                                             **kwargs):
    return "uniforms" in (log_message.message.lower()
                          if log_message.message is not None else "")


class UniformsgetActiveUniformMutator(BaseMutator):
    def __init__(self):
        self.name = "UniformsgetActiveUniformMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createProgram")
        print("UniformsgetActiveUniform")
        return True

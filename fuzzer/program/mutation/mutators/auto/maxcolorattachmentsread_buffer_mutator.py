...
_api_names = "readBuffer"


def maxcolorattachmentsread_buffer_mutator_match(program, api, log_message,
                                                 **kwargs):
    return "max_color_attachments" in (log_message.message.lower() if
                                       log_message.message is not None else "")


class MaxcolorattachmentsreadBufferMutator(BaseMutator):
    def __init__(self):
        self.name = "MaxcolorattachmentsreadBufferMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(0,
                        api.args[0].arg_type.gen_value(None, api, api.args[0]))
        print("MaxcolorattachmentsreadBuffer")
        return True

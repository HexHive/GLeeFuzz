...
_api_names = "drawBuffers"


def colorattachmentiextdraw_buffers_mutator_match(program, api, log_message,
                                                  **kwargs):
    return "color_attachmenti_ext" in (log_message.message.lower() if
                                       log_message.message is not None else "")


class ColorattachmentiextdrawBuffersMutator(BaseMutator):
    def __init__(self):
        self.name = "ColorattachmentiextdrawBuffersMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(0,
                        api.args[0].arg_type.gen_value(None, api, api.args[0]))
        print("ColorattachmentiextdrawBuffers")
        return True

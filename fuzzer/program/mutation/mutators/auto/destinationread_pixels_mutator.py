...
def destinationread_pixels_mutator_match(program, api, log_message, **kwargs):
    return "destination" in (log_message.message.lower()
                             if log_message.message is not None else "")


class DestinationreadPixelsMutator(BaseMutator):
    def __init__(self):
        self.name = "DestinationreadPixelsMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(4,
                        api.args[4].arg_type.gen_value(None, api, api.args[4]))
        print("DestinationreadPixels")
        return True

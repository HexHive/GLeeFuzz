...
def contextdelete_vertex_array_mutator_match(program, api, log_message,
                                             **kwargs):
    return "context" in (log_message.message.lower()
                         if log_message.message is not None else "")


class ContextdeleteVertexArrayMutator(BaseMutator):
    def __init__(self):
        self.name = "ContextdeleteVertexArrayMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name(
            "createVertexArray")
        print("ContextdeleteVertexArray")
        return True

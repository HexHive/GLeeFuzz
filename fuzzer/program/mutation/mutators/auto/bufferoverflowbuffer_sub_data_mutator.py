...
_api_names = "bufferSubData"


def bufferoverflowbuffer_sub_data_mutator_match(program, api, log_message,
                                                **kwargs):
    return "buffer overflow" in (log_message.message.lower()
                                 if log_message.message is not None else "")


class BufferoverflowbufferSubDataMutator(BaseMutator):
    def __init__(self):
        self.name = "BufferoverflowbufferSubDataMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(1,
                        api.args[1].arg_type.gen_value(None, api, api.args[1]))
        print("BufferoverflowbufferSubData")
        return True

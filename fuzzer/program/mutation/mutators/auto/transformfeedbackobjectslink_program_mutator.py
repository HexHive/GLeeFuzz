...
_api_names = "linkProgram"


def transformfeedbackobjectslink_program_mutator_match(program, api,
                                                       log_message, **kwargs):
    return "transform feedback objects" in (
        log_message.message.lower() if log_message.message is not None else "")


class TransformfeedbackobjectslinkProgramMutator(BaseMutator):
    def __init__(self):
        self.name = "TransformfeedbackobjectslinkProgramMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createProgram")
        print("TransformfeedbackobjectslinkProgram")
        return True

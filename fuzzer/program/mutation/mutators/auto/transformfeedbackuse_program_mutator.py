...
_api_names = "useProgram"


def transformfeedbackuse_program_mutator_match(program, api, log_message,
                                               **kwargs):
    return "transform feedback" in (log_message.message.lower()
                                    if log_message.message is not None else "")


class TransformfeedbackuseProgramMutator(BaseMutator):
    def __init__(self):
        self.name = "TransformfeedbackuseProgramMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createProgram")
        print("TransformfeedbackuseProgram")
        return True

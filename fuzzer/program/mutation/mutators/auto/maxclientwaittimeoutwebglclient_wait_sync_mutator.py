...
def maxclientwaittimeoutwebglclient_wait_sync_mutator_match(
        program, api, log_message, **kwargs):
    return "max_client_wait_timeout_webgl" in (
        log_message.message.lower() if log_message.message is not None else "")


class MaxclientwaittimeoutwebglclientWaitSyncMutator(BaseMutator):
    def __init__(self):
        self.name = "MaxclientwaittimeoutwebglclientWaitSyncMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(2,
                        api.args[2].arg_type.gen_value(None, api, api.args[2]))
        print("MaxclientwaittimeoutwebglclientWaitSync")
        return True

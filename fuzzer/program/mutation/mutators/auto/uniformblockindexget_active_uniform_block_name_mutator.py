...
_api_names = "getActiveUniformBlockName"


def uniformblockindexget_active_uniform_block_name_mutator_match(
        program, api, log_message, **kwargs):
    return "uniform block index" in (log_message.message.lower() if
                                     log_message.message is not None else "")


class UniformblockindexgetActiveUniformBlockNameMutator(BaseMutator):
    def __init__(self):
        self.name = "UniformblockindexgetActiveUniformBlockNameMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(1,
                        api.args[1].arg_type.gen_value(None, api, api.args[1]))
        print("UniformblockindexgetActiveUniformBlockName")
        return True

...
_api_names = "getAttribLocation"


def programget_attrib_location_mutator_match(program, api, log_message,
                                             **kwargs):
    return "program" in (log_message.message.lower()
                         if log_message.message is not None else "")


class ProgramgetAttribLocationMutator(BaseMutator):
    def __init__(self):
        self.name = "ProgramgetAttribLocationMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createProgram")
        print("ProgramgetAttribLocation")
        return True

...
_api_names = "bindAttribLocation"


def prefixbind_attrib_location_mutator_match(program, api, log_message,
                                             **kwargs):
    return "prefix" in (log_message.message.lower()
                        if log_message.message is not None else "")


class PrefixbindAttribLocationMutator(BaseMutator):
    def __init__(self):
        self.name = "PrefixbindAttribLocationMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createProgram")
        print("PrefixbindAttribLocation")
        return True

...
_api_names = "getActiveAttrib"


def attributesget_active_attrib_mutator_match(program, api, log_message,
                                              **kwargs):
    return "attributes" in (log_message.message.lower()
                            if log_message.message is not None else "")


class AttributesgetActiveAttribMutator(BaseMutator):
    def __init__(self):
        self.name = "AttributesgetActiveAttribMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createProgram")
        print("AttributesgetActiveAttrib")
        return True

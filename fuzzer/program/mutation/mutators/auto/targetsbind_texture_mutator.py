...
def targetsbind_texture_mutator_match(program, api, log_message, **kwargs):
    return "targets" in (log_message.message.lower()
                         if log_message.message is not None else "")


class TargetsbindTextureMutator(BaseMutator):
    def __init__(self):
        self.name = "TargetsbindTextureMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(0,
                        api.args[0].arg_type.gen_value(None, api, api.args[0]))
        print("TargetsbindTexture")
        return True

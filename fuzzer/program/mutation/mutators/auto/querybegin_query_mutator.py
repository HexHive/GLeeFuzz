...
_api_names = "beginQuery"


def querybegin_query_mutator_match(program, api, log_message, **kwargs):
    return "query" in (log_message.message.lower()
                       if log_message.message is not None else "")


class QuerybeginQueryMutator(BaseMutator):
    def __init__(self):
        self.name = "QuerybeginQueryMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createQuery")
        print("QuerybeginQuery")
        return True

...
_api_names = "vertexAttrib4fv"


def arrayvertex_attribfv_mutator_match(program, api, log_message, **kwargs):
    return "array" in (log_message.message.lower()
                       if log_message.message is not None else "")


class ArrayvertexAttribfvMutator(BaseMutator):
    def __init__(self):
        self.name = "ArrayvertexAttribfvMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(1,
                        api.args[1].arg_type.gen_value(None, api, api.args[1]))
        print("ArrayvertexAttribfv")
        return True

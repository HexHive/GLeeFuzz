...
_api_names = "deleteFramebuffer"


def opaqueframebufferdelete_framebuffer_mutator_match(program, api,
                                                      log_message, **kwargs):
    return "opaque framebuffer" in (log_message.message.lower()
                                    if log_message.message is not None else "")


class OpaqueframebufferdeleteFramebufferMutator(BaseMutator):
    def __init__(self):
        self.name = "OpaqueframebufferdeleteFramebufferMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name(
            "createFramebuffer")
        print("OpaqueframebufferdeleteFramebuffer")
        return True

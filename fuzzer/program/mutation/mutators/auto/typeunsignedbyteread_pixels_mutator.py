...
def typeunsignedbyteread_pixels_mutator_match(program, api, log_message,
                                              **kwargs):
    return "type unsigned_byte" in (log_message.message.lower()
                                    if log_message.message is not None else "")


class TypeunsignedbytereadPixelsMutator(BaseMutator):
    def __init__(self):
        self.name = "TypeunsignedbytereadPixelsMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(5,
                        api.args[5].arg_type.gen_value(None, api, api.args[5]))
        print("TypeunsignedbytereadPixels")
        return True

...
_api_names = "uniform4ui"


def locationuniformui_mutator_match(program, api, log_message, **kwargs):
    return "location" in (log_message.message.lower()
                          if log_message.message is not None else "")


class LocationuniformuiMutator(BaseMutator):
    def __init__(self):
        self.name = "LocationuniformuiMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name(
            "getUniformLocation")
        print("Locationuniformui")
        return True

...
_api_names = "drawElements"


def bufferdraw_elements_mutator_match(program, api, log_message, **kwargs):
    return "buffer" in (log_message.message.lower()
                        if log_message.message is not None else "")


class BufferdrawElementsMutator(BaseMutator):
    def __init__(self):
        self.name = "BufferdrawElementsMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(3,
                        api.args[3].arg_type.gen_value(None, api, api.args[3]))
        print("BufferdrawElements")
        return True

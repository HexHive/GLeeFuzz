...
_api_names = "disableVertexAttribArray"


def rangedisable_vertex_attrib_array_mutator_match(program, api, log_message,
                                                   **kwargs):
    return "range" in (log_message.message.lower()
                       if log_message.message is not None else "")


class RangedisableVertexAttribArrayMutator(BaseMutator):
    def __init__(self):
        self.name = "RangedisableVertexAttribArrayMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(0,
                        api.args[0].arg_type.gen_value(None, api, api.args[0]))
        print("RangedisableVertexAttribArray")
        return True

...
_api_names = "getUniformLocation"


def programget_uniform_location_mutator_match(program, api, log_message,
                                              **kwargs):
    return "program" in (log_message.message.lower()
                         if log_message.message is not None else "")


class ProgramgetUniformLocationMutator(BaseMutator):
    def __init__(self):
        self.name = "ProgramgetUniformLocationMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createProgram")
        print("ProgramgetUniformLocation")
        return True

...
def uniformlocationget_uniform_mutator_match(program, api, log_message,
                                             **kwargs):
    return "uniformlocation" in (log_message.message.lower()
                                 if log_message.message is not None else "")


class UniformlocationgetUniformMutator(BaseMutator):
    def __init__(self):
        self.name = "UniformlocationgetUniformMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name(
            "getUniformLocation")
        print("UniformlocationgetUniform")
        return True

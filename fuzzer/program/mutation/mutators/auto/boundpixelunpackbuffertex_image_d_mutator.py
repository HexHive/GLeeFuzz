...
def boundpixelunpackbuffertex_image_d_mutator_match(program, api, log_message,
                                                    **kwargs):
    return "bound pixel_unpack_buffer" in (
        log_message.message.lower() if log_message.message is not None else "")


class BoundpixelunpackbuffertexImageDMutator(BaseMutator):
    def __init__(self):
        self.name = "BoundpixelunpackbuffertexImageDMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(8,
                        api.args[8].arg_type.gen_value(None, api, api.args[8]))
        print("BoundpixelunpackbuffertexImageD")
        return True

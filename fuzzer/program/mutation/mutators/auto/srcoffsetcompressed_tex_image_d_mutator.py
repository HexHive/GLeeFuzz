...
def srcoffsetcompressed_tex_image_d_mutator_match(program, api, log_message,
                                                  **kwargs):
    return "srcoffset" in (log_message.message.lower()
                           if log_message.message is not None else "")


class SrcoffsetcompressedTexImageDMutator(BaseMutator):
    def __init__(self):
        self.name = "SrcoffsetcompressedTexImageDMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(8,
                        api.args[8].arg_type.gen_value(None, api, api.args[8]))
        print("SrcoffsetcompressedTexImageD")
        return True

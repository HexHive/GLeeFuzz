...
_api_names = "texImage3D"


def texturestex_image_d_mutator_match(program, api, log_message, **kwargs):
    return "textures" in (log_message.message.lower()
                          if log_message.message is not None else "")


class TexturestexImageDMutator(BaseMutator):
    def __init__(self):
        self.name = "TexturestexImageDMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(2,
                        api.args[2].arg_type.gen_value(None, api, api.args[2]))
        print("TexturestexImageD")
        return True

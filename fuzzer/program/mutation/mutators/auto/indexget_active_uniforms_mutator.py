...
def indexget_active_uniforms_mutator_match(program, api, log_message,
                                           **kwargs):
    return "index" in (log_message.message.lower()
                       if log_message.message is not None else "")


class IndexgetActiveUniformsMutator(BaseMutator):
    def __init__(self):
        self.name = "IndexgetActiveUniformsMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name(
            "getUniformIndices")
        print("IndexgetActiveUniforms")
        return True

...
_api_names = "getActiveUniforms"


def activeuniformsget_active_uniforms_mutator_match(program, api, log_message,
                                                    **kwargs):
    return "active_uniforms" in (log_message.message.lower()
                                 if log_message.message is not None else "")


class ActiveuniformsgetActiveUniformsMutator(BaseMutator):
    def __init__(self):
        self.name = "ActiveuniformsgetActiveUniformsMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name(
            "getUniformIndices")
        print("ActiveuniformsgetActiveUniforms")
        return True

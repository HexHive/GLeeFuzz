...
_api_names = "uniform3i"


def locationuniformi_mutator_match(program, api, log_message, **kwargs):
    return "location" in (log_message.message.lower()
                          if log_message.message is not None else "")


class LocationuniformiMutator(BaseMutator):
    def __init__(self):
        self.name = "LocationuniformiMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name(
            "getUniformLocation")
        print("Locationuniformi")
        return True

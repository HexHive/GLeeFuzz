...
_api_names = "getTransformFeedbackVarying"


def indexget_transform_feedback_varying_mutator_match(program, api,
                                                      log_message, **kwargs):
    return "index" in (log_message.message.lower()
                       if log_message.message is not None else "")


class IndexgetTransformFeedbackVaryingMutator(BaseMutator):
    def __init__(self):
        self.name = "IndexgetTransformFeedbackVaryingMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(1,
                        api.args[1].arg_type.gen_value(None, api, api.args[1]))
        print("IndexgetTransformFeedbackVarying")
        return True

...
def arraybuffervertex_attrib_pointer_mutator_match(program, api, log_message,
                                                   **kwargs):
    return "array_buffer" in (log_message.message.lower()
                              if log_message.message is not None else "")


class ArraybuffervertexAttribPointerMutator(BaseMutator):
    def __init__(self):
        self.name = "ArraybuffervertexAttribPointerMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(5,
                        api.args[5].arg_type.gen_value(None, api, api.args[5]))
        print("ArraybuffervertexAttribPointer")
        return True

...
def errorget_uniform_mutator_match(program, api, log_message, **kwargs):
    return "error" in (log_message.message.lower()
                       if log_message.message is not None else "")


class ErrorgetUniformMutator(BaseMutator):
    def __init__(self):
        self.name = "ErrorgetUniformMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name("createProgram")
        print("ErrorgetUniform")
        return True

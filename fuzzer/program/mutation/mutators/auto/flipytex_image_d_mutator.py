...
def flipytex_image_d_mutator_match(program, api, log_message, **kwargs):
    return "flip_y" in (log_message.message.lower()
                        if log_message.message is not None else "")


class FlipytexImageDMutator(BaseMutator):
    def __init__(self):
        self.name = "FlipytexImageDMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(1,
                        api.args[1].arg_type.gen_value(None, api, api.args[1]))
        print("FlipytexImageD")
        return True

...
def premultiplyalphaisnttex_sub_image_d_mutator_match(program, api,
                                                      log_message, **kwargs):
    return "premultiply_alpha isn't" in (
        log_message.message.lower() if log_message.message is not None else "")


class PremultiplyalphaisnttexSubImageDMutator(BaseMutator):
    def __init__(self):
        self.name = "PremultiplyalphaisnttexSubImageDMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(0,
                        api.args[0].arg_type.gen_value(None, api, api.args[0]))
        print("PremultiplyalphaisnttexSubImageD")
        return True

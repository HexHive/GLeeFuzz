...
_api_names = "beginTransformFeedback"


def transformfeedbackvaryingsbegin_transform_feedback_mutator_match(
        program, api, log_message, **kwargs):
    return "transform feedback varyings" in (
        log_message.message.lower() if log_message.message is not None else "")


class TransformfeedbackvaryingsbeginTransformFeedbackMutator(BaseMutator):
    def __init__(self):
        self.name = "TransformfeedbackvaryingsbeginTransformFeedbackMutator"

    def _mutate(self, program, api, **kwargs):
        api.set_arg_val(0,
                        api.args[0].arg_type.gen_value(None, api, api.args[0]))
        print("TransformfeedbackvaryingsbeginTransformFeedback")
        return True

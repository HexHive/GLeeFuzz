...
_api_names = "transformFeedbackVaryings"


def varyingstransform_feedback_varyings_mutator_match(program, api,
                                                      log_message, **kwargs):
    return "varyings" in (log_message.message.lower()
                          if log_message.message is not None else "")


class VaryingstransformFeedbackVaryingsMutator(BaseMutator):
    def __init__(self):
        self.name = "VaryingstransformFeedbackVaryingsMutator"

    def _mutate(self, program, api, **kwargs):
        program.gen_args_for_api_and_add_to_program_by_name(
            "getSupportedExtensions")
        print("VaryingstransformFeedbackVaryings")
        return True
